...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private UUID oni, testParticipant, oniBot, forcedOni, configuredFriendlyNpcId, oniBotTacticalTarget; private final Set<UUID> oniTeam=new HashSet<>(); private final Map<UUID,String> rolePreferences=new HashMap<>(); private OniType oniType=OniType.DAKKO, selectedOniType; private int brokenHearts, totalHearts, requiredHearts, secondsLeft; private BukkitTask ticker, sidebarTicker, chaseTicker, repairTicker, staminaTicker, botTicker, bgmTicker, gameStartBgmTicker, dawnTicker, crimsonTicker, finalEffectTestTicker; private UUID finalEffectTestPlayer; private boolean finalPhase, playerSideTest; private String activeBgmSound, activeGameStartBgmSound;
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("mode")){if(a.length<2){msg(sender,"現在のモード: &e"+(getConfig().getString("game-mode","normal").equalsIgnoreCase("duo")?"二人鬼":"通常"));return true;}if(state!=GameState.WAITING){msg(sender,"&cゲーム中はモードを変更できません。");return true;}String m=a[1].toLowerCase(Locale.ROOT);if(!m.equals("normal")&&!m.equals("duo")){msg(sender,"/og mode <normal|duo>");return true;}getConfig().set("game-mode",m);saveConfig();all("&eゲームモードを &f"+(m.equals("duo")?"二人鬼（鬼2 / ぷれいやー12）":"通常")+" &eに変更しました。");return true;}
    ...
    private void start(OniType forced,CommandSender starter,boolean playerTest){
        ...
        OniType upcomingOniType=forced!=null?forced:(selectedOniType!=null?selectedOniType:new OniType[]{OniType.DAKKO,OniType.KISHIN,OniType.SHIKKI,OniType.YUUKI,OniType.KANKI}[new Random().nextInt(5)]);
        ...
        boolean duoMode=getConfig().getString("game-mode","normal").equalsIgnoreCase("duo")&&!playerTest;
    ...
    private boolean isOni(UUID id){return id!=null&&(oniTeam.contains(id)||(oni!=null&&oni.equals(id))||(oniBot!=null&&oniBot.equals(id)));}
    ...
    private void openOniMenu(Player p){
        ...
        if(isJakutsukiLocked()){menu.setItem(8,customModelItem(Material.TOTEM_OF_UNDYING,0,"&5&l鬼神 &7(オニガミ)","menu_oni:KANKI","&7召喚・指揮型","&5鬼喚び &7/ &d鬼令 &7/ &6再臨 &7/ &4百鬼夜行",shown==OniType.KANKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(12,customModelItem(Material.CHAINMAIL_HELMET,1001,"&5&l堕狐","menu_oni:DAKKO","&7奇襲・攪乱型","&d狐渡り &7/ &6分霊 &7/ &6狐火 &7/ &d天来",shown==OniType.DAKKO?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(14,customModelItem(Material.CARVED_PUMPKIN,1203,"&4&l鬼王 &7(きおう)","menu_oni:KISHIN","&7正面突破・追跡型","&c鬼突 &7/ &4地砕 &7/ &4鬼吼 &7/ &6剛身",shown==OniType.KISHIN?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(16,customModelItem(Material.CARVED_PUMPKIN,1201,"&c&l疾鬼","menu_oni:SHIKKI","&7跳躍・高速機動・連続狩猟型","&6跳躍狩り &7/ &c血走 &7/ &c狂奔 &7/ &4狩猟連鎖",shown==OniType.SHIKKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(10,customModelItem(Material.CARVED_PUMPKIN,1202,"&8&l幽鬼","menu_oni:YUUKI","&7隠密・奇襲型","&8幽歩 &7/ &7朧渡り &7/ &5影縫い &7/ &8神隠し",shown==OniType.YUUKI?"&a現在表示・選択中":"&eクリックして選択"));}
        else{menu.setItem(7,customModelItem(Material.TOTEM_OF_UNDYING,0,"&5&l鬼神 &7(オニガミ)","menu_oni:KANKI","&7召喚・指揮型","&5鬼喚び &7/ &d鬼令 &7/ &6再臨 &7/ &4百鬼夜行",shown==OniType.KANKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(11,customModelItem(Material.CHAINMAIL_HELMET,1001,"&5&l堕狐","menu_oni:DAKKO","&7奇襲・攪乱型","&d狐渡り &7/ &6分霊 &7/ &6狐火 &7/ &d天来",shown==OniType.DAKKO?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(13,customModelItem(Material.PLAYER_HEAD,2009,"&8&lEX鬼 蛇窟姫 &7(ジャクツキ)","menu_oni:JAKUTSUKI","&7領域制圧・終盤強化型","&5黒鏡 &7/ &5薙ぎ払い &7/ &8蛇崩 &7/ &4解放 &7/ &5黒の波動","&c基礎体力200 / 心臓破壊ごとに最大体力-10","&c固有ルール: 鬼の心臓が8個に増加","&4偽心臓4個 &7――接近すると爆発","&7爆発は地形を破壊せず、ダメージ＋発光を付与","&f全心臓破壊時: 全ぷれいやーへ『蛇斬』支給","&6救援: 残存4人以下 または 心臓残り3個でアザクジ参戦",shown==OniType.JAKUTSUKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(15,customModelItem(Material.CARVED_PUMPKIN,1203,"&4&l鬼王 &7(きおう)","menu_oni:KISHIN","&7正面突破・追跡型","&c鬼突 &7/ &4地砕 &7/ &4鬼吼 &7/ &6剛身",shown==OniType.KISHIN?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(17,customModelItem(Material.CARVED_PUMPKIN,1201,"&c&l疾鬼","menu_oni:SHIKKI","&7跳躍・高速機動・連続狩猟型","&6跳躍狩り &7/ &c血走 &7/ &c狂奔 &7/ &4狩猟連鎖",shown==OniType.SHIKKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(9,customModelItem(Material.CARVED_PUMPKIN,1202,"&8&l幽鬼","menu_oni:YUUKI","&7隠密・奇襲型","&8幽歩 &7/ &7朧渡り &7/ &5影縫い &7/ &8神隠し",shown==OniType.YUUKI?"&a現在表示・選択中":"&eクリックして選択"));}
    ...
    private void updateOniBot(){
        ...
        if(oniType==OniType.DAKKO){
            ...
            else{Vector leap=predictedTarget.toVector().subtract(bot.getLocation().toVector()).normalize().multiply(getConfig().getDouble("shikki-skills.hunting-leap.bot-power",1.45));leap.setY(Math.max(.48,leap.getY()+.48));bot.setVelocity(leap);bot.setFallDistance(0);bot.getWorld().playSound(bot.getLocation(),Sound.ENTITY_ENDER_DRAGON_FLAP,.7f,1.55f);botNextAbilityAt=now+oniBotAbilityCooldownMillis(getConfig().getInt("shikki-skills.hunting-leap.bot-cooldown-seconds",9));}
    ...
    private void applyGameNight(World world){
        ...
        debugTrace("world","WRD-501","apply night world="+world.getName()+" time=18000 daylightCycle=false");
    ...
    private void restoreLobbyDay(World world){
        if(world==null)return;
        debugTrace("world","WRD-502","restore lobby day world="+world.getName()+" time=1000 daylightCycle=true");
        world.setStorm(false);
        world.setThundering(false);
        world.setTime(1000L);
    ...
    private void useShikkiHuntingLeap(Player p){
        ...
        if(!ready(p,"shikki_hunting_leap",cd))return;
        ...
        Vector v=p.getLocation().getDirection().normalize().multiply(power);v.setY(Math.max(.48,v.getY()+.48));p.setVelocity(v);p.setFallDistance(0);
        shikkiLeapAttackUntil.put(p.getUniqueId(),System.currentTimeMillis()+Math.max(500,getConfig().getLong("shikki-skills.hunting-leap.attack-window-millis",3000)));
        p.getWorld().playSound(p.getLocation(),Sound.ENTITY_ENDER_DRAGON_FLAP,.8f,1.55f);p.getWorld().spawnParticle(Particle.CLOUD,p.getLocation(),24,.35,.15,.35,.07);
        p.sendActionBar(cc(shikkiFrenzyActive()?"&c狂奔跳躍 &7――命中で次の跳躍を即時回復":"&6跳躍狩り &7――命中で即時再使用"));
    }
